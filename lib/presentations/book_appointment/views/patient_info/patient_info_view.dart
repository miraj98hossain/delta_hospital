...
import 'package:delta_hospital/presentations/book_appointment/views/appointment_info/appointment_info_page.dart';
...
class _PatientInfoViewState extends State<PatientInfoView> {
  ...
  late HisPatientInfo patientInfo;
  ...
  void initState() {
    ...
    patientInfo = HisPatientInfo();
    if (widget.patientType.patientTypeNo == "2") {
      context.read<HisPatientInfoBloc>().add(
            HisPatientInfoGet(
              mrnOrPhNo: widget.patientPortalUser?.mrn ?? '0',
            ),
          );
    }
        ...
        child: BlocBuilder<HisPatientInfoBloc, HisPatientInfoState>(
          builder: (context, state) {
            if (state is HisPatientInfoSuccess) {
              ...
              _nameController.text = patientInfo.patientName?.trim() ?? '';
              _phoneController.text = patientInfo.phoneMobile?.trim() ?? '';
              _emailController.text = patientInfo.email?.trim() ?? '';
              _addressController.text = patientInfo.address?.trim() ?? '';

              var gender = Gender.values.where((element) {
                return element.value == patientInfo.genderData;
              }).toList();
              if (gender.isNotEmpty) {
                context.read<VariableStateCubit<Gender>>().update(gender[0]);
              }
              try {
                var date = DateTime.parse(patientInfo.dob ?? '');
                        ...
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              ...
                              IgnorePointer(
                                ignoring: widget.patientPortalUser != null &&
                                        context
                                                .watch<
                                                    VariableStateCubit<
                                                        Gender>>()
  ...
  bool _validate() {
    var selectedDob = context.read<VariableStateCubit<DateTime>>().state;
    if (selectedDob == null) {
