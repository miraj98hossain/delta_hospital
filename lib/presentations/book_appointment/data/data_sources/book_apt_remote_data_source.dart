...
import 'package:delta_hospital/presentations/book_appointment/data/models/consultation_fee_response.dart';
...
import 'package:delta_hospital/presentations/book_appointment/data/models/slot_status_response.dart';
...
class BookAptRemoteDataSourceImpl
  ...
  Future<AppointmentBookingResponse> bookAppointment({
    ...
    var request = http.Request(
        'POST',
        Uri.parse(
            '${_appConfig.baseUrl}online-appointment-api/fapi/appointment/bookAppointment'));
    request.body = json.encode(appointment.toMap());
    request.headers.addAll(headers);

    http.StreamedResponse response = await request.send();

    return await decodeResponse(response,
        decoder: AppointmentBookingResponse.fromJson);
  }
}
